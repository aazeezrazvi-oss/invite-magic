...
export const InvitationCoreSchema = z.object({
  ...
  groom_photo: z.string().optional().or(z.literal('')).transform(sanitizeUrl),
  groom_bio: z.string().max(500).optional().or(z.literal('')).transform(sanitizeText),
  bride_name: z.string().min(1).max(100).transform(sanitizeText),
  bride_photo: z.string().optional().or(z.literal('')).transform(sanitizeUrl),
  bride_bio: z.string().max(500).optional().or(z.literal('')).transform(sanitizeText),
  parents_names: z.string().max(200).optional().or(z.literal('')).transform(sanitizeText),
  invitation_message: z.string().max(2000).optional().or(z.literal('')).transform(sanitizeText),
  template_id: z.string().uuid().optional().nullable(),
  custom_domain: z.string().max(100).optional().nullable().transform(val => val ? sanitizeUrl(val) : null),
  is_published: z.boolean().default(false),
  gallery_photos: z.array(z.string().transform(sanitizeUrl)).optional().default([]),
});
